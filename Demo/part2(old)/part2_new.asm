...
;***********************************************************************
;**  Einblenden, GFX, 1x1 und 4x4 Scroller                            **
;**  Ausbleden aller Elemente nach Schwarz                            **
;***********************************************************************
;** ScreenRAM $4400-$47e8
...
.m12:       lda #$00
            ...
            ; IRQ Routine Teil 1.2 ------------------------------------------]
            ; Mitte Bildschirm, blauer Balken -------------------------------]
            ; ---------------------------------------------------------------]
.p1irq2:    inc VIC2InteruptStatus     ; acknowledge interrupt
            ldx #$03
            dex
            bne *-1
.c20:       lda #$00
...
.p1update_irq:
            ...
            ; IRQ Routine Teil 1.3 ------------------------------------------]
            ; grüner Balken -------------------------------------------------]
            ; ---------------------------------------------------------------]
.p1irq3b:   inc VIC2InteruptStatus     ; acknowledge interrupt
            lda #$D1
            cmp $D012
            bne *-3
            ldx #$0b
...
.c41:       lda #$00
            ...
            ; IRQ Routine Teil 1.4 ------------------------------------------]
            ; Untere Kante des Bildschirms ----------------------------------]
            ; ---------------------------------------------------------------]
.p1irq4:    inc VIC2InteruptStatus     ; acknowledge interrupt
            lda #$F8
            cmp $d012
            bne *-3
            ldx #$0B
...
.mc2:       lda #$03
            ...
            ; IRQ Routine Teil 2.2 ------------------------------------------]
            ; Unterhalb der Grafik, blauer Balken ---------------------------]
            ; ---------------------------------------------------------------]
...
.subroutine:
            ...
            ; IRQ Routine 2.3b ----------------------------------------------]
            ; grüner Balken -------------------------------------------------]
.p2irq3b:   inc VIC2InteruptStatus     ; acknowledge interrupt
            lda #$D1
            cmp $D012
            bne *-3
            ldx #$0b
...
.p2C10:     lda #$05    ; green
            ...
            ; IRQ Routine Teil 2.4 ------------------------------------------]
            ; unterer Bildschirm --------------------------------------------]
.p2irq4:    inc VIC2InteruptStatus     ; acknowledge interrupt
            lda #$F8
            cmp $d012
            bne *-3
            ldx #$0B
...
+           rts
            ...
            ; 4x4 Scroller --------------------------------------------------]
            ; ---------------------------------------------------------------]
.scroll4x4: lda .scroll_delay_4x4
            sec
...
+           stx .fade_count
            ...
            ; Variablen und Zwischenspeicher --------------------------------]
            ; ---------------------------------------------------------------]
.counter:           !byte $00
