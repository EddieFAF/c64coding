...
; Fade Parameter
fade_speed = 1 ; Verzögerung (01=schnell, 04=langsam)
...
!if .RELEASE {
        !set C_APPLY_INTERRUPT = apply_interrupt
    } else { 
        !set C_APPLY_INTERRUPT = .APPLY_INTERRUPT
    }
...
!byte $0c,$08
!word 1971
!byte $9e,$32,$30,$36,$34,$00,$00,$00,$00
...
+       rts
        ...
        ; Universal Fade Routine --------------------------------------------]
        ; -------------------------------------------------------------------]
        ; Input: startzeile, endzeile, speed --------------------------------]
        ; Aufruf: jsr fade --------------------------------------------------]
        ; Derzeitiger Bereich: $2000-$3f40 ----------------------------------]
        ; -------------------------------------------------------------------]
!zone fade {
        ; Fadeout downward --------------------------------------------------]
        ; -------------------------------------------------------------------]
...
.zh     !byte >$2000,>$2140,>$2280,>$23c0,>$2500,>$2640,>$2780,>$28C0
        ...
        !byte >$3400,>$3540,>$3680,>$37C0,>$3900,>$3A40,>$3B80,>$3CC0
        !byte >$3E00
.zeile_up  !byte endzeile
.zeile_dn  !byte startzeile
.counter_up: !byte $07
.counter_dn: !byte $00
.zaehler: !byte $00
