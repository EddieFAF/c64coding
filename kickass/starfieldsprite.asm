...
clr2000:
    ...
    ldx #$00
setsprs:
    lda #$80    //Sprite object data from $2000-$2080
    sta $07f8,x
    ...
    ldx #$00
positions:
    lda postable,x //Read label postable
    ...
    cli
mainloop:
    lda #$00 //Synchronize the routines outside IRQ so that all routines run outside IRQ
    sta sync //correctly
    lda sync
waitsync:
    cmp sync
    ...
    jmp waitsync
cont:
    jsr expdpos     //Call label xpdpos for sprite position x expansion
...
expdpos:
    ldx #$00
xpdloop:
    lda starpos+1,x //Read virtual memory from starpos (odd number values)
...
irq:
    inc $d019 //You should also know this bit already
    lda #$00
    sta $d012
    lda #$01
    sta sync
